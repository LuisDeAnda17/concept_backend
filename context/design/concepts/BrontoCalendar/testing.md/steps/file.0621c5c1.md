...
content_id: 0621c5c187c8c30e3b16e20faa4592321531e2159e721f0ab3ddbd441e70c01a
---
 ...
 *                If you create a calendar for a user, then create and add an assignment to it,
 ...
 *                The ability to manage office hours similarly, including changing their scheduled time,
 *                is also central to the calendar's utility.
 */
export default class BrontoCalendarConcept {
  ...
  async removeWork({ owner, assignmentId }: { owner: User; assignmentId: AssignmentRef }): Promise<Empty | { error: string }> {
    ...
    // Precondition 2: Find the assignment in the concept's state (needed to determine its original due date)
    const assignment = await this.assignments.findOne({ _id: assignmentId });
    if (!assignment) {
      return { error: `Assignment with ID ${assignmentId} not found in BrontoCalendar's state.` };
    }
    ...
    if (updateResult.matchedCount === 0) {
      // If the day doc wasn't found for the correct calendar, or it didn't contain the assignment.
    ...
    if (oldNormalizedDueDate !== newNormalizedDueDate) {
      ...
      await this.calendarDays.updateOne(
        { _id: newDayKey, calendarId: calendar._id },
        {
          ...
          $setOnInsert: { calendarId: calendar._id, date: newDueDate, officeHours: [] },
        },
        { upsert: true }
      );
    } else {
        // If date component didn't change, but time might have, and we still want to ensure it's on the calendar.
        // This is effectively re-calling assignWork for the same day.
        // It's benign due to $addToSet, but ensures consistency if assignment was somehow missing from the day.
        const currentDayKey = `${calendar._id}_${newNormalizedDueDate}` as CalendarDayRef;
        await this.calendarDays.updateOne(
  ...
  async changeOH({ owner, officeHoursId, newDate, newDuration }: { owner: User; officeHoursId: OfficeHoursRef; newDate: Date; newDuration: number }): Promise<Empty | { error: string }> {
    ...
    if (oldNormalizedDate !== newNormalizedDate) {
      ...
      await this.calendarDays.updateOne(
        ...
        // If date component didn't change, but time might have, and we still want to ensure it's on the calendar.
        // This is effectively re-calling assignOH for the same day.
        // It's benign due to $addToSet, but ensures consistency if OH was somehow missing from the day.
