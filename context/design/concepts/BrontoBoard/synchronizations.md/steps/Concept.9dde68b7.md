...
content_id: 9dde68b73f6d82c13d006d14a91946f1f2a03752f401f655d532138d23bb49e8
---

# Concept: BrontoBoard (Updated)
...
export default class BrontoBoardConcept {
   ...
   * (Internal to concept, not directly exposed via syncs as a query)
  ...
  async initializeBB(input: {
    owner: User; // Renamed from 'user' to 'owner' for consistency with internal concept docs
    calendar: Calendar;
  }): Promise<{ brontoBoard: ID } | { error: string }> {
    const { owner, calendar } = input;
  ...
  // --- Concept Queries for Syncs ---

  /**
   * _query: _isBrontoBoardOwner
   * Checks if a given user is the owner of a BrontoBoard.
   * @param input An object containing brontoBoard ID and user ID.
   *   - `brontoBoard`: The ID of the BrontoBoard.
   *   - `user`: The ID of the user to check.
   * @returns An array containing the BrontoBoard ID if the user is the owner, otherwise an empty array.
   */
  async _isBrontoBoardOwner(input: { brontoBoard: ID; user: User }): Promise<{ brontoBoard: ID }[]> {
    const { brontoBoard: brontoBoardId, user: userId } = input;
    const brontoBoard = await this.brontoBoards.findOne({ _id: brontoBoardId, owner: userId });
    return brontoBoard ? [{ brontoBoard: brontoBoardId }] : [];
  }

  /**
   * _query: _isClassOwner
   * Checks if a given user is the owner of the BrontoBoard that a class belongs to.
   * @param input An object containing class ID and user ID.
   *   - `class`: The ID of the class.
   *   - `user`: The ID of the user to check.
   * @returns An array containing the Class ID if the user is the owner of its BrontoBoard, otherwise an empty array.
   */
  async _isClassOwner(input: { class: ID; user: User }): Promise<{ class: ID }[]> {
    const { class: classId, user: userId } = input;
    const classDoc = await this.classes.findOne({ _id: classId });
    if (!classDoc) {
      return []; // Class not found
    }
    // Check if the user is the owner of the BrontoBoard associated with this class
    const brontoBoard = await this.brontoBoards.findOne({ _id: classDoc.brontoBoardId, owner: userId });
    return brontoBoard ? [{ class: classId }] : [];
  }

  /**
   * _query: _getAssignmentById
   * Retrieves an assignment by its ID.
   * @param input An object containing the assignment ID.
   *   - `assignment`: The ID of the assignment.
   * @returns An array containing the AssignmentDoc (with 'class' as classId) if found, otherwise an empty array.
   */
  async _getAssignmentById(input: { assignment: ID }): Promise<{ assignment: ID; class: ID }[]> {
    const { assignment: assignmentId } = input;
    const doc = await this.assignments.findOne({ _id: assignmentId });
    // Return with a 'class' key for consistency with sync variable binding
    return doc ? [{ assignment: doc._id, class: doc.classId }] : [];
  }

  /**
   * _query: _getOfficeHourById
   * Retrieves office hours by its ID.
   * @param input An object containing the office hour ID.
   *   - `officeHour`: The ID of the office hour.
   * @returns An array containing the OfficeHourDoc (with 'class' as classId) if found, otherwise an empty array.
   */
  async _getOfficeHourById(input: { officeHour: ID }): Promise<{ officeHour: ID; class: ID }[]> {
    const { officeHour: officeHourId } = input;
    const doc = await this.officeHours.findOne({ _id: officeHourId });
    // Return with a 'class' key for consistency with sync variable binding
    return doc ? [{ officeHour: doc._id, class: doc.classId }] : [];
  }
